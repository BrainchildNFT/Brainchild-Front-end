...
function CustomToggle({ children, eventKey, callback, title }) {
    ...
    return ( <
        button className = "w-full bg-gray-darkest p-8 mb-1 sm:mb-2 text-left"
        type = "button"
        ...
        <
        span className = "text-pink-light font-normal font-sora text-lg sm:text-2xl" > { title } <
        /span> <
        embed className = "ml-4 transition-all duration-700 pointer-events-none"
...
const Faq = forwardRef((_, ref) => {
    ...
    return ( <
        div className = "relative min-h-screen flex items-center overflow-x-scroll overflow-y-hidden no-scrollbar mt-screen-1/4 select-none"
        ref = { ref.containerRef } >
        <
        div className = "sm:p-24 w-full container mx-auto"
        ref = { ref.contentRef } >
        <
        ...
        /div> {
            content.map((value) =>
                ...
                <
                div className = "w-full p-8 sm:p-16 font-cormorant font-semibold leading-tight sm:leading-tight text-2xl sm:text-4xl italic background-blur mb-1 sm:mb-2" > { value.desc } <
                /div> <
                /Accordion.Collapse> <
                /Accordion>
            )
